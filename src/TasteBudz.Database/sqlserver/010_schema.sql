...
BEGIN
    CREATE TABLE dbo.Restaurants (
        ...
        IsArchived BIT NOT NULL CONSTRAINT DF_Restaurants_IsArchived DEFAULT 0
    );
END;
GO
...
IF OBJECT_ID(N'dbo.PasswordResetRequests', N'U') IS NOT NULL
   AND COL_LENGTH(N'dbo.Restaurants', N'StreetAddress') IS NOT NULL
   AND COL_LENGTH(N'dbo.Restaurants', N'IsArchived') IS NOT NULL
   AND NOT EXISTS (SELECT 1 FROM dbo.SchemaVersions WHERE Version = N'20260422-password-reset-requests-and-restaurant-catalog')
BEGIN
    INSERT INTO dbo.SchemaVersions (Version, Description)
