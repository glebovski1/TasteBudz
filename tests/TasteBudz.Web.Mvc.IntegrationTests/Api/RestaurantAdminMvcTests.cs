...
public sealed class RestaurantAdminMvcTests
{
    ...
    public async Task UpdateSlot_PostsEditedSlotToBackend()
    {
        ...
        factory.BackendHandler.Enqueue(
            ...
            (_, _) => StubBackendApiHandler.Json(
                HttpStatusCode.OK,
                new RestaurantSlotDto(slotId, restaurantId, new DateTimeOffset(2026, 5, 1, 18, 30, 0, TimeSpan.Zero), new DateTimeOffset(2026, 5, 1, 22, 30, 0, TimeSpan.Zero), 6, new DateTimeOffset(2026, 5, 1, 17, 30, 0, TimeSpan.Zero), 4, RestaurantSlotStatus.Open, DateTimeOffset.UtcNow, DateTimeOffset.UtcNow, null, null)));
